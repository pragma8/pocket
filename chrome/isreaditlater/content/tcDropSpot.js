...
window.DropSpot = function(bounds, options) {
  try {
    if (typeof options == 'undefined')
      options = {};
    ...
    var $container = iQ('<div>')
      .addClass('drop-spot')
      .css({top:'50%',right:0,width:48,height:48})
      .appendTo('body');
...
// ----------
window.DropSpot.superclass = InfoItem.prototype;
window.DropSpot.prototype = Utils.extend(new InfoItem(), new Subscribable(), { 
    ...
    setBounds: function(rect, immediately)
    {
        ...
        window.DropSpot.superclass.setBounds.call(this, rect, immediately);
    },
    
    overlapsWithOtherItems: function()
    {
        return false; // it's okay to overlap with a dropspot
    },
    
    // The default getBounds just grabs the stored this.bounds value which isn't accurate if it was changed by other means
    getBounds: function()
    {       
        return new Rect(iQ(this.container).bounds());
    },
});
